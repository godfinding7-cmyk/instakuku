...
async function extractJson(reelUrl) {
  const attempts = [
    // Fast path first: the native HTTP handler has less overhead when Instagram accepts it.
    [...commonArgs(), '--dump-single-json', reelUrl],
    // Fallback only when the normal request is blocked/challenged.
  ...
  for (const args of attempts) {
    try {
      ...
      return {
        ...
        method: args.includes('--impersonate') ? 'browser-like fallback' : 'fast standard request'
      };
    } catch (e) {
      lastErr = e;
    }
